import time
from astrbot.api.all import (
    Star, Context, logger,
    Plain, Json, Poke, At, Reply,
    AstrBotConfig, AstrMessageEvent
)
from astrbot.api.event import filter
from .RulesClass import *
from .Tools import 帮助文本, 获取所有指令, 概率通过, 解析黑白名单, 检测黑白名单

op = time.perf_counter()


class 群自定义规则(Star):

    def __init__(self, context: Context, config: AstrBotConfig):

        super().__init__(context)
        self.config = config
        self.异常 = False
        # ======获取配置======
        try:
            self.规则列表: list[自定义规则] = config.get('自定义规则', [])
            self.所有指令 = 获取所有指令(self.config['额外指令'])
            self.所有指令集合 = set(self.所有指令)
            self.兜底规则: 兜底规则 = config['兜底规则']
            self.兜底规则['备注'] = "兜底规则"
            self.兜底开关 = self.兜底规则['开关']
            self.黑名单群聊 = [i.strip() for i in config['黑名单群聊']]
            config['黑名单群聊'] = self.黑名单群聊
        except Exception as e:
            logger.critical(
                f"【群唤醒增强】获取配置出错，请尝试点击插件配置然后直接保存，或重新安装插件，或联系开发者\n{str(e)}",
                exc_info=True)
            self.异常 = True
        # ======获取系统配置======
        try:
            self.指令前缀 = tuple(context.get_config()["wake_prefix"])
        except Exception as e:
            logger.error(f"【群唤醒增强】获取指令前缀失败，你可在代码里手动设置，使用默认值 '/': {e}")
            self.指令前缀 = ("/",)

        logger.info(f"【群唤醒增强】所有{len(self.所有指令)}个指令：\n{self.所有指令}")

        self.规则索引 = {}
        self.群组活跃间隔 = {}
        for 索引, 规则 in enumerate(self.规则列表):
            规则['群号'] = [j.strip() for j in 规则['群号']]
            for 群号 in 规则['群号']:
                if 群号 in self.规则索引:
                    logger.warning(f"【群唤醒增强】群 {群号} 已定义，请检查")
                    continue
                self.规则索引[群号] = 索引
                self.群组活跃间隔[群号] = 规则['活跃间隔']

        for i, j in self.群组活跃间隔.items():
            if j == -1:
                self.群组活跃间隔[i] = self.兜底规则['活跃间隔']

        self.系统指令 = {'alter_cmd', 'dashboard_update', 'del', 'deop', 'dwl', 'groupnew', 'help', 'history', 'key',
                        'llm', 'ls', 'model', 'new', 'op', 'persona', 'provider', 'rename', 'reset', 'sid', 'switch',
                        't2i', 'tts', 'wl'}

        # 群聊级活跃状态
        self.群组活跃时间 = {}
        self.群组上次唤醒时间 = {}
        # 用户级活跃状态
        self.用户活跃时间 = {}  # 用户ID -> 活跃到期时间戳
        self.用户上次唤醒时间 = {}  # 用户ID -> 上次唤醒时间戳（用于活跃间隔检查）
        # 用户自定义活跃状态（群组隔离，key=(群号, 用户ID)）
        self.用户自定义活跃时间 = {}  # (群号, 用户ID) -> 到期时间戳
        self.用户自定义活跃时长 = {}  # (群号, 用户ID) -> 设置的秒数

        self.用户冷却时间 = {}

        ed = time.perf_counter()
        耗时 = ed - op
        logger.info(f"【群唤醒增强】启动完成，耗时{耗时:.6f}秒")

    @filter.on_astrbot_loaded()
    async def 启动获取所有指令(self):
        """框架初次启动完成时获取所有指令"""
        self.所有指令 = 获取所有指令(self.config['额外指令'])
        self.所有指令集合 = set(self.所有指令)
        logger.info(f"\n\n【群唤醒增强】所有{len(self.所有指令)}个指令：\n{self.所有指令}\n\n")

    @filter.event_message_type(filter.EventMessageType.GROUP_MESSAGE, priority=6666)
    async def 入口(self, event: AstrMessageEvent):
        """消息主入口"""
        try:
            if not (消息链 := event.get_messages()):
                规则_tmp = self.获取当前群规(event)
                if 规则_tmp and 规则_tmp.get('拦截空消息链事件', False):
                    self.终止事件传播(event, 规则_tmp)
                return

            event.set_extra("群唤醒拦截", False)

            群号 = event.get_group_id()
            当前时间 = time.time()

            if 群号 in self.黑名单群聊:
                self.终止事件传播(event)
                return

            发送者 = event.get_sender_id()

            规则 = self.获取当前群规(event)
            if not 规则:
                return

            # ========== 用户黑白名单检查 ==========
            if not self.用户名单通过(发送者, 规则):
                if not event.is_admin() or 规则.get('禁用功能对管理员生效', False):
                    logger.info(f"【群唤醒增强】「{规则['备注']}」用户「{发送者}」未通过用户黑白名单")
                    self.终止事件传播(event, 规则)
                    return

            # ========== 用户自定义活跃检查 ==========
            自定义活跃键 = (群号, 发送者)
            if 自定义活跃键 in self.用户自定义活跃时间:
                if 当前时间 < self.用户自定义活跃时间[自定义活跃键]:
                    间隔 = 规则['活跃间隔']
                    if 间隔 == -1:
                        间隔 = self.兜底规则['活跃间隔']
                    上次唤醒 = self.用户上次唤醒时间.get(发送者, 0)
                    if 当前时间 - 上次唤醒 < 间隔:
                        logger.info(f"【群唤醒增强】用户「{发送者}」自定义活跃期内触发冷却拦截")
                        self.终止事件传播(event)
                        return
                    else:
                        logger.info(f"【群唤醒增强】用户「{发送者}」自定义活跃期内唤醒")
                        self.用户上次唤醒时间[发送者] = 当前时间
                        # 用自定义时长重置活跃期
                        自定义时长 = self.用户自定义活跃时长.get(自定义活跃键, 0)
                        if 自定义时长 > 0:
                            self.用户自定义活跃时间[自定义活跃键] = 当前时间 + 自定义时长
                        event.is_at_or_wake_command = True
                        return
                else:
                    del self.用户自定义活跃时间[自定义活跃键]
                    # 不删时长，用户可以再次被唤醒后继续使用

            终止, ret = self.指令屏蔽(event, 规则)
            if 终止 and (not event.is_admin() or 规则.get('禁用功能对管理员生效', False)):
                logger.info(f"【群唤醒增强】「{规则['备注']}」触发了指令拦截")
                self.终止事件传播(event, 规则)
                return
            if ret:
                return

            if isinstance(消息链[0], Json):
                if 规则['拦截json']:
                    logger.info(f"【群唤醒增强】「{规则['备注']}」触发了json拦截（外部分享）")
                    self.终止事件传播(event, 规则)
                    return

            if isinstance(消息链[0], Poke):
                if 规则['拦截戳一戳']:
                    logger.info(f"【群唤醒增强】「{规则['备注']}」触发了戳一戳拦截")
                    self.终止事件传播(event, 规则)
                return

            # 用户冷却检查（独立于活跃范围）
            if 发送者 in self.用户冷却时间 and (not event.is_admin() or 规则.get('禁用功能对管理员生效', False)):
                if 当前时间 < self.用户冷却时间[发送者]:
                    logger.info(f"【群唤醒增强】用户「{发送者}」触发了冷却拦截")
                    self.终止事件传播(event)
                    return
                else:
                    del self.用户冷却时间[发送者]

            活跃范围 = 规则['活跃范围']

            # ========== 活跃状态检查 ==========
            if 活跃范围 == '群聊级':
                # 群聊级活跃检查
                if 群号 in self.群组活跃时间:
                    if self.处理艾特引用(event, 规则):
                        return
                    if 当前时间 < self.群组活跃时间[群号]:
                        上次唤醒 = self.群组上次唤醒时间.get(群号, 0)
                        间隔 = self.群组活跃间隔.get(群号, self.兜底规则['活跃间隔'])
                        if 当前时间 - 上次唤醒 < 间隔:
                            logger.info(f"【群唤醒增强】「{规则['备注']}」群聊级活跃期内触发冷却拦截")
                            self.终止事件传播(event, 规则)
                            return
                        else:
                            logger.info(f"【群唤醒增强】「{规则['备注']}」群聊级活跃期内唤醒")
                            self.唤醒(event, 规则)
                            return
                    else:
                        del self.群组活跃时间[群号]
                        self.群组上次唤醒时间.pop(群号, None)
            else:
                # 用户级活跃检查
                if 发送者 in self.用户活跃时间:
                    if self.处理艾特引用(event, 规则):
                        return
                    if 当前时间 < self.用户活跃时间[发送者]:
                        上次唤醒 = self.用户上次唤醒时间.get(发送者, 0)
                        间隔 = 规则['活跃间隔']
                        if 间隔 == -1:
                            间隔 = self.兜底规则['活跃间隔']
                        if 当前时间 - 上次唤醒 < 间隔:
                            logger.info(f"【群唤醒增强】「{规则['备注']}」用户级活跃期内触发冷却拦截")
                            self.终止事件传播(event, 规则)
                            return
                        else:
                            logger.info(f"【群唤醒增强】「{规则['备注']}」用户级活跃期内唤醒")
                            self.唤醒(event, 规则)
                            return
                    else:
                        del self.用户活跃时间[发送者]
                        self.用户上次唤醒时间.pop(发送者, None)

            # ========== 正常唤醒流程（昵称、艾特、概率等） ==========
            消息文本 = event.get_message_str()

            if any(_ in 消息文本 for _ in 规则['昵称唤醒']):
                logger.info(f"【群唤醒增强】「{规则['备注']}」触发了昵称唤醒")
                self.唤醒(event, 规则)
                return

            if 规则['前缀拦截'] and any(消息文本.startswith(_) for _ in 规则['前缀拦截']):
                logger.info(f"【群唤醒增强】「{规则['备注']}」触发了前缀拦截")
                self.终止事件传播(event, 规则)
                return

            if 规则['含有拦截'] and any(_ in 消息文本 for _ in 规则['含有拦截']):
                logger.info(f"【群唤醒增强】「{规则['备注']}」触发了含有拦截")
                self.终止事件传播(event, 规则)
                return

            if self.处理艾特引用(event, 规则):
                return

            if 规则['其余拦截']:
                if event.is_admin() and not 规则.get('禁用功能对管理员生效', False):
                    return
                logger.info(f"【群唤醒增强】「{规则['备注']}」触发了其余拦截")
                self.终止事件传播(event, 规则)
                return

            if 规则['概率唤醒']:
                if 概率通过(消息链, 规则['概率唤醒'], 规则['概率方式']):
                    logger.info(f"【群唤醒增强】「{规则['备注']}」触发了概率唤醒")
                    self.唤醒(event, 规则)
            return
        except (KeyError, AttributeError) as e:
            if self.异常:
                logger.error(
                    "【群唤醒增强】运行出错，原因：插件配置未正确加载。请进入插件配置页面，直接点击【保存】一次即可恢复正常。如未恢复正常，请重新安装插件")
                return
            logger.error(f"【群唤醒增强】消息处理出现错误：{e}", exc_info=True)
        except Exception as e:
            logger.error(f"【群唤醒增强】消息处理出现错误：{e}", exc_info=True)

    def 用户名单通过(self, 发送者, 规则) -> bool:
        """检查用户是否通过当前规则的用户黑白名单"""
        原列表 = 规则.get('用户黑白名单', ['all']) or ['all']
        return 检测黑白名单(str(发送者), 解析黑白名单(原列表))

    def 获取当前群规(self, event: AstrMessageEvent) -> 自定义规则 | 兜底规则 | None:
        群号 = event.get_group_id()
        if not 群号:
            return None
        if 群号 in self.规则索引:
            规则: 自定义规则 = self.规则列表[self.规则索引[群号]]
            if not 规则['开关']:
                return None
        elif self.兜底开关:
            规则: 兜底规则 = self.兜底规则
        else:
            return None
        return 规则

    def 获取事件规则(self, event: AstrMessageEvent) -> dict:
        """返回当前事件时间类所属的规则键值对"""
        pass

    @filter.on_llm_request(priority=66666)
    async def llm请求前(self, event: AstrMessageEvent, _):
        """兜底拦截"""
        try:
            规则 = self.获取当前群规(event)
            if not 规则:
                return
            if not 规则['强力拦截']:
                return
            终止, ret = self.指令屏蔽(event, 规则, True)
            if 终止 or ret:
                logger.info(f"【群唤醒增强】「{规则['备注']}」触发了指令拦截")
                self.终止事件传播(event, 规则)
                logger.info(
                    f"【群唤醒增强】群{规则['备注']}，用户「{event.get_sender_name()}」，消息 | {event.get_message_outline()} | 触发了指令拦截")
                return
            elif event.get_extra("群唤醒拦截"):
                event.stop_event()
                logger.info(
                    f"【群唤醒增强】群{规则['备注']}，用户「{event.get_sender_name()}」，消息 | {event.get_message_outline()} | 拦截了llm唤醒")
                logger.info(f"拦截了此次事件，{event.get_extra('群唤醒拦截')}")
            else:
                logger.info(
                    f"【群唤醒增强】群{规则['备注']}，用户「{event.get_sender_name()}」，消息 | {event.get_message_outline()} | 唤醒了llm")
        except (KeyError, AttributeError) as e:
            if self.异常:
                logger.error(
                    "【群唤醒增强】运行出错，原因：插件配置未正确加载。请进入插件配置页面，直接点击【保存】一次即可恢复正常。如未恢复正常，请重新安装插件")
                return
            logger.error(f"【群唤醒增强】出错：\n{e}", exc_info=True)
        except Exception as e:
            logger.error(f"【群唤醒增强】拦截出错：\n{e}", exc_info=True)

    @filter.on_llm_response()
    async def llm请求后(self, event: AstrMessageEvent, _):
        """当有 LLM 请求后的事件"""
        try:
            event.set_extra("群唤醒llm请求后", True)
            规则 = self.获取当前群规(event)
            if not 规则:
                return
            if 规则['活跃方式'] == 'llm请求后':
                self.记录活跃(event, 规则)
        except (KeyError, AttributeError) as e:
            if self.异常:
                logger.error(
                    "【群唤醒增强】运行出错，原因：插件配置未正确加载。请进入插件配置页面，直接点击【保存】一次即可恢复正常。如未恢复正常，请重新安装插件")
                return
            logger.error(f"【群唤醒增强】出错：\n{e}", exc_info=True)
        except Exception as e:
            logger.error(f"【群唤醒增强】出错：\n{e}", exc_info=True)

    @filter.after_message_sent()
    async def 发送消息后(self, event: AstrMessageEvent):
        """在消息发送后的事件"""
        try:
            if not event.get_extra("群唤醒llm请求后", False):
                return
            if event.get_extra("群唤醒已处理", False):
                return
            event.set_extra("群唤醒已处理", True)
            规则 = self.获取当前群规(event)
            if not 规则:
                return
            if 规则['活跃方式'] == '发送消息后':
                self.记录活跃(event, 规则)
        except (KeyError, AttributeError) as e:
            if self.异常:
                logger.error(
                    "【群唤醒增强】运行出错，原因：插件配置未正确加载。请进入插件配置页面，直接点击【保存】一次即可恢复正常。如未恢复正常，请重新安装插件")
                return
            logger.error(f"【群唤醒增强】出错：\n{e}", exc_info=True)
        except Exception as e:
            logger.error(f"【群唤醒增强】出错：\n{e}", exc_info=True)

    def 记录活跃(self, event: AstrMessageEvent, 规则):
        """根据活跃范围记录群组或用户的活跃状态"""
        当前时间 = time.time()
        持续活跃时间 = 规则['持续活跃']
        if 持续活跃时间 == -1:
            持续活跃时间 = self.兜底规则['持续活跃']

        群号 = event.get_group_id()
        发送者 = event.get_sender_id()
        自定义活跃键 = (群号, 发送者)

        # 用户自定义活跃时长覆盖规则时长
        if 自定义活跃键 in self.用户自定义活跃时长:
            自定义时长 = self.用户自定义活跃时长[自定义活跃键]
            if 自定义时长 > 0:
                logger.info(f"【群唤醒增强】用户「{event.get_sender_name()}」使用自定义活跃时长{自定义时长}秒")
                self.用户自定义活跃时间[自定义活跃键] = 当前时间 + 自定义时长
                self.用户上次唤醒时间[发送者] = 当前时间
                return

        if 持续活跃时间 <= 0:
            return

        活跃范围 = 规则['活跃范围']
        if 活跃范围 == '群聊级':
            logger.info(f"【群唤醒增强】「{规则['备注']}」群聊级将持续活跃{持续活跃时间}秒")
            self.群组活跃时间[群号] = 当前时间 + 持续活跃时间
            self.群组上次唤醒时间[群号] = 当前时间
        else:
            logger.info(f"【群唤醒增强】「{规则['备注']}」用户「{event.get_sender_name()}」用户级将持续活跃{持续活跃时间}秒")
            self.用户活跃时间[发送者] = 当前时间 + 持续活跃时间
            self.用户上次唤醒时间[发送者] = 当前时间

    @staticmethod
    def 终止事件传播(event: AstrMessageEvent, 规则: Rule = None):
        if 规则 is None:
            规则 = {}
        if 规则.get('强力拦截', False):
            event.set_extra("群唤醒拦截", True)
        logger.info(f"【群唤醒增强】「{规则.get('备注', event.get_group_id())}」终止了事件传播")
        event.stop_event()

    def 处理艾特引用(self, event: AstrMessageEvent, 规则: Rule) -> bool:
        """返回值决定调用后上一级是否也return"""
        艾特 = False
        引用 = False

        for seg in event.get_messages():
            if isinstance(seg, At) and str(seg.qq) == event.get_self_id():
                艾特 = True
                break
            if isinstance(seg, Reply) and str(seg.sender_id) == event.get_self_id():
                引用 = True

        if 艾特 or 引用:
            if 规则['引用唤醒'] and 艾特 and 引用:
                logger.info(f"【群唤醒增强】「{规则['备注']}」触发了艾特引用唤醒")
                self.唤醒(event, 规则)
                return True
            if 规则['艾特唤醒'] and 艾特:
                logger.info(f"【群唤醒增强】「{规则['备注']}」触发了艾特唤醒")
                self.唤醒(event, 规则)
                return True
            if 规则['无艾特引用唤醒'] and 引用:
                logger.info(f"【群唤醒增强】「{规则['备注']}」触发了无艾特引用唤醒")
                self.唤醒(event, 规则)
                return True
            logger.info(f"【群唤醒增强】「{规则['备注']}」拦截了此次艾特或引用唤醒")
            self.终止事件传播(event, 规则)
            return True
        return False

    def 唤醒(self, event: AstrMessageEvent, 规则):
        """设置唤醒状态"""
        event.is_at_or_wake_command = True
        # 记录活跃（根据活跃方式）
        if 规则['活跃方式'] == "唤醒时":
            self.记录活跃(event, 规则)

        # 设置用户冷却（唤醒CD）
        if event.is_admin() and not 规则.get('禁用功能对管理员生效', False):
            return
        唤醒CD = 规则['唤醒CD']
        if 唤醒CD == -1:
            唤醒CD = self.兜底规则['唤醒CD']
        if 唤醒CD > 0:
            self.用户冷却时间[event.get_sender_id()] = time.time() + 唤醒CD
            logger.info(f"【群唤醒增强】用户「{event.get_sender_name()}（{event.get_sender_id()}）」将冷却{唤醒CD}秒")

    def 指令屏蔽(self, event: AstrMessageEvent, 规则, llm请求前:bool=False) -> tuple[bool, bool]:
        """检查指令是否被屏蔽，return终止，return"""
        指令文本 = event.get_message_str().strip().split()
        指令文本 = 指令文本[0] if 指令文本 else ""
        ret = 指令文本 in self.所有指令集合

        使用前缀 = False
        if next((seg.text for seg in event.get_messages() if isinstance(seg, Plain)), '').strip().startswith(
                self.指令前缀):
            使用前缀 = True

        if 规则['禁用系统指令'] and 指令文本 in self.系统指令:
            return True, ret

        if 规则['禁用的指令']:
            if '0所有' in 规则['禁用的指令']:
                if 使用前缀:
                    return True, ret
                elif 指令文本 in self.所有指令集合:
                    return True, ret
                else:
                    return False, ret
            elif 指令文本 in 规则['禁用的指令']:
                return True, ret

        if 规则['启用的指令']:
            if '0所有' in 规则['启用的指令']:
                return False, ret
            elif 指令文本 not in 规则['启用的指令']:
                return True, ret

        if 使用前缀 and 规则['禁前唤醒']:
            if llm请求前:
                return True, ret
            elif 指令文本 not in self.所有指令集合:
                return True, ret

        return False, ret

    #@filter.command("所有指令")
    #@filter.permission_type(filter.PermissionType.ADMIN)
    async def 指令菜单(self, event: AstrMessageEvent):
        """查看所有指令"""
        yield event.plain_result('/' + '\n/'.join(self.所有指令))

    #@filter.command("刷新指令")
    #@filter.permission_type(filter.PermissionType.ADMIN)
    async def 刷新指令(self, event: AstrMessageEvent):
        """刷新指令"""
        刷新前 = len(self.所有指令)
        self.所有指令 = 获取所有指令(self.config['额外指令'])
        self.所有指令集合 = set(self.所有指令)
        差值 = len(self.所有指令) - 刷新前
        yield event.plain_result(f"✅ 已刷新所有指令，新增{差值}个")

    @filter.command("设置群规")
    @filter.permission_type(filter.PermissionType.ADMIN)
    async def 设置群规(self, event: AstrMessageEvent, 键: str, 值: str, 群号: str = None, 操作: str = "null"):
        """设置群规配置项，用法：/设置群规 <键> <值> [群号] [操作]
        操作参数仅对列表类型有效：add(追加，默认)、del(删除)、rep(替换)
        列表值使用逗号分割多个值
        示例：
            /设置群规 开关 on
            /设置群规 昵称唤醒 机器人,AI
            /设置群规 昵称唤醒 机器人 add        # 追加“机器人”
            /设置群规 昵称唤醒 机器人 del        # 删除“机器人”
            /设置群规 昵称唤醒 AI助手 rep        # 替换为仅“AI助手”
            /设置群规 禁用的指令 help,status rep
            /设置群规 概率唤醒 0.3
            /设置群规 持续活跃 60
            /设置群规 艾特唤醒 off 兜底规则
        """
        原群号 = 群号
        if 群号 is not None:
            群号 = str(群号).strip()
            if 群号.lower() in ('add', 'del', 'rep') and 操作 == "null":
                操作 = 群号.lower()
                群号 = None

        if 群号 is None:
            群号 = event.get_group_id()
            if not 群号:
                yield event.plain_result("❌ 请在群聊中使用，或通过参数指定群号（或“兜底规则”）")
                return
        群号 = str(群号).strip()
        键 = str(键).strip()
        值 = str(值).strip()

        规则列表 = self.config.get('自定义规则', [])
        规则 = None
        来源 = None

        if 群号 == '兜底规则':
            规则 = self.config['兜底规则']
            if not 规则:
                yield event.plain_result("❌ 未找到兜底规则配置")
                return
            来源 = "兜底规则"
        else:
            for r in 规则列表:
                if 群号 in r['群号']:
                    规则 = r
                    来源 = f"群 {群号}"
                    break
            if not 规则:
                yield event.plain_result(f"❌ 未找到群 {群号} 的规则，请先使用 /添加群规 添加")
                return

        if 键 not in 规则:
            可用的键 = list(规则.keys())
            if '__template_key' in 可用的键:
                可用的键.remove('__template_key')
            if 可用的键:
                tips = '\n'.join(可用的键)
            else:
                tips = '无可用键'
            yield event.plain_result(f"❌ 规则中不存在键「{键}」，可用的键有：\n{tips}")
            return

        原值 = 规则[键]
        原类型 = type(原值)
        try:
            if 原类型 is bool:
                if 操作 != "null":
                    yield event.plain_result(f"⚠️ 键「{键}」为布尔类型，不支持操作参数「{操作}」，已忽略")
                值小写 = 值.lower()
                if 值小写 in ('开', 'true', '1', 'on'):
                    转换后 = True
                elif 值小写 in ('关', 'false', '0', 'off'):
                    转换后 = False
                else:
                    raise ValueError(f"无法解析为布尔值: {值}，支持：开/关，1/0，on/off，true/false")
            elif 原类型 is float:
                if 操作 != "null":
                    yield event.plain_result(f"⚠️ 键「{键}」为浮点数类型，不支持操作参数「{操作}」，已忽略")
                转换后 = float(值)
                if 键 == "概率唤醒" and not (0.0 < 转换后 < 1.0):
                    yield event.plain_result("❌ 值超出范围，概率值应在0~1之间")
                    return
            elif 原类型 is list:
                值 = 值.replace("，", ",")
                if 值.strip() == "null":
                    新列表 = []
                else:
                    新列表 = [item.strip() for item in 值.split(',') if item.strip()]
                实际操作 = "add" if 操作 == "null" else 操作
                if 实际操作 == "add":
                    if 键 == '概率方式':
                        活跃方式 = ["纯文本", "卡片分享", "图片", "任何消息"]
                        not_exist = [x for x in 新列表 if x not in 活跃方式]
                        if not_exist:
                            yield event.plain_result(
                                f"❌ 不支持的值：{'，'.join(not_exist)}\n\n该键支持的有：{'，'.join(活跃方式)}")
                            return
                    转换后 = 原值 + 新列表
                    转换后 = list(dict.fromkeys(转换后))
                elif 实际操作 == "del":
                    not_exist = [x for x in 新列表 if x not in 原值]
                    if not_exist:
                        yield event.plain_result(f"❌ 原列表中不存在：{'，'.join(not_exist)}")
                        return
                    转换后 = [item for item in 原值 if item not in 新列表]
                elif 实际操作 == "rep":
                    转换后 = 新列表
                else:
                    yield event.plain_result(f"❌ 不支持的操作类型: {操作}，请使用 add/del/rep")
                    return
            elif 原类型 is str:
                if 操作 != "null":
                    yield event.plain_result(f"⚠️ 键「{键}」为字符串类型，不支持操作参数「{操作}」，已忽略")
                转换后 = 值
                if 键 == '活跃方式' and 转换后 not in ("唤醒时", "llm请求后", "发送消息后"):
                    yield event.plain_result("❌ 不支持的值，该键支持的值：唤醒时/llm请求后/发送消息后")
                    return
                if 键 == '活跃范围' and 转换后 not in ("群聊级", "用户级"):
                    yield event.plain_result("❌ 不支持的值，该键支持的值：群聊级/用户级")
                    return
            else:
                yield event.plain_result(f"❌ 不支持的类型: {原类型}")
                return
        except Exception as e:
            yield event.plain_result(f"⚠️ 类型转换失败：{e}\n期望类型：{原类型.__name__}")
            return

        规则[键] = 转换后
        self.config.save_config()
        self._重建规则索引()

        输出行 = [f"✅ 已更新 {来源} 的规则：{键} = {转换后}"]
        if 操作 in ('add', 'del', 'rep') and 原类型 is list:
            输出行.append(f"• 操作模式：{操作}")
        if len(规则.get('群号', [])) > 1:
            if 原群号 is None:
                输出行.append(f"• 还有{len(规则['群号']) - 1}个群也使用了此规则")
            else:
                输出行.append(f"• 还有群{规则['群号']}也使用了此规则")
        if 规则['开关'] is False and 键 != '开关':
            输出行.append("• ⚠️ 该规则已关闭，若要使用请开启开关")

        yield event.plain_result('\n'.join(输出行))

    @filter.command("添加群规")
    @filter.permission_type(filter.PermissionType.ADMIN)
    async def 添加群规(self, event: AstrMessageEvent, 群号: str = None, 备注: str = None):
        """为指定群号添加新的自定义规则"""
        if 群号 is None:
            群号列表 = [event.get_group_id()]
        else:
            群号 = str(群号)
            群号列表 = [g.strip() for g in 群号.split(',') if g.strip()]
            if not 群号列表:
                yield event.plain_result("❌ 请提供有效的群号")
                return

        已存在群号 = []
        for g in 群号列表:
            if g in self.规则索引:
                已存在群号.append(g)
        if 已存在群号:
            yield event.plain_result(f"❌ 群号{已存在群号}已存在自定义规则，请使用 /设置群规 修改")
            return

        规则模板 = self.config.schema['自定义规则']['templates']['自定义']['items']
        默认规则 = {key: item['default'] for key, item in 规则模板.items()}
        默认规则['__template_key'] = '自定义'
        新规则 = 默认规则.copy()
        新规则['群号'] = 群号列表
        if not 备注 and len(群号列表) == 1:
            try:
                备注 = (await event.get_group(群号列表[0])).group_name
            except Exception as e:
                logger.warning(e, exc_info=True)
            if not 备注:
                备注 = f"群 {'，'.join(群号列表)} 的自定义规则"
        新规则['备注'] = 备注

        规则列表 = self.config.get('自定义规则', [])
        规则列表.append(新规则)
        self.config['自定义规则'] = 规则列表
        self.config.save_config()
        self._重建规则索引()

        yield event.plain_result(
            f"✅ 已为以下群号添加自定义规则：{', '.join(群号列表)}\n"
            f"备注：{新规则['备注']}\n"
            f"可使用 /设置群规 修改具体配置项。/群规则 查看当前群规")

    def _重建规则索引(self):
        self.规则索引.clear()
        self.群组活跃间隔.clear()
        self.规则列表 = self.config.get('自定义规则', [])
        if not self.规则列表:
            return
        for 索引, 规则 in enumerate(self.规则列表):
            规则['群号'] = [j.strip() for j in 规则['群号']]
            for 群号 in 规则['群号']:
                if 群号 in self.规则索引:
                    logger.warning(f"【群唤醒增强】群 {群号} 已存在规则，跳过重复")
                    continue
                self.规则索引[群号] = 索引
                self.群组活跃间隔[群号] = 规则['活跃间隔']
        for 群号, 间隔 in self.群组活跃间隔.items():
            if 间隔 == -1:
                self.群组活跃间隔[群号] = self.兜底规则['活跃间隔']

    @filter.command("群规则")
    @filter.permission_type(filter.PermissionType.ADMIN)
    async def 查看群规则(self, event: AstrMessageEvent, 群号: str = None):
        """查看指定群聊的规则配置"""
        原群号 = 群号
        if 群号 is None:
            if not (群号 := event.get_group_id()):
                yield event.plain_result("❌ 请在群里中使用或传入群号参数")
                return
        群号 = str(群号).strip()

        if not self.规则列表 and 群号 != "兜底规则":
            yield event.plain_result("⚠️ 你还没有配置任何自定义规则")
            return

        if 群号 in self.规则索引:
            规则 = self.规则列表[self.规则索引[群号]]
            来源 = "自定义规则"
        elif 群号 == "兜底规则":
            规则 = self.兜底规则
            来源 = "兜底规则"
        elif self.兜底开关:
            规则 = self.兜底规则
            来源 = "兜底规则"
        else:
            yield event.plain_result(f"❌ 未找到群 {群号} 的规则配置（兜底规则已关闭）")
            return

        def f(键) -> str:
            if 规则[键] == -1:
                return f"• {键}：{self.兜底规则[键]}秒（使用兜底值）" # type: ignore
            else:
                return f"• {键}：{规则[键]}秒" # type: ignore

        输出行 = [f"📋 群 {群号} 的规则配置（{来源}）", f"• 备注：{规则['备注']}",
                f"• 总开关：{'✅ 开启' if 规则['开关'] else '❌ 关闭'}",
                f"• 昵称唤醒：{', '.join(规则['昵称唤醒']) or '无'}",
                f"• 艾特唤醒：{'✅' if 规则['艾特唤醒'] else '❌'}",
                f"• 引用唤醒：{'✅' if 规则['引用唤醒'] else '❌'}",
                f"• 无艾特引用唤醒：{'✅' if 规则['无艾特引用唤醒'] else '❌'}",
                f"• 前缀拦截：{', '.join(规则['前缀拦截']) or '无'}",
                f"• 含有拦截：{', '.join(规则['含有拦截']) or '无'}",
                f"• 其余拦截：{'✅' if 规则['其余拦截'] else '❌'}",
                f"• 拦截JSON：{'✅' if 规则['拦截json'] else '❌'}",
                f"• 拦截戳一戳：{'✅' if 规则['拦截戳一戳'] else '❌'}",
                f"• 概率唤醒：{规则['概率唤醒'] * 100:.0f}%",
                f"• 概率方式：{', '.join(规则['概率方式']) or '无'}",
                f('持续活跃'),
                f('活跃间隔'),
                f('唤醒CD'),
                f"• 活跃方式：{规则['活跃方式']}",
                f"• 活跃范围：{规则['活跃范围']}",
                f"• 禁前唤醒：{'✅' if 规则['禁前唤醒'] else '❌'}",
                f"• 禁用系统指令：{'✅' if 规则['禁用系统指令'] else '❌'}",
                f"• 禁用的指令：{', '.join(规则['禁用的指令']) or '未配置'}",
                f"• 启用的指令：{', '.join(规则['启用的指令']) or '未配置'}",
                f"• 用户黑白名单：{', '.join(规则.get('用户黑白名单', ['all'])) or '未配置'}",
                f"• 强力拦截：{'✅' if 规则['强力拦截'] else '❌'}"]
        if len(规则.get('群号', [])) > 1:
            if 原群号 is None:
                输出行.append(f"• 还有{len(规则['群号']) - 1}个群也使用了此规则")
            else:
                输出行.append(f"• 还有群{规则['群号']}也使用了此规则")

        yield event.plain_result('\n'.join(输出行))

    @filter.command("删除群规")
    @filter.permission_type(filter.PermissionType.ADMIN)
    async def 删除群规(self, event: AstrMessageEvent, 群号: str = None):
        """删除指定群号的自定义规则，或从多群号规则中移除该群"""
        if 群号 is None:
            if not (群号 := event.get_group_id()):
                yield event.plain_result("❌ 请在群聊中使用，或通过参数指定群号")
                return
        群号 = str(群号).strip()

        if 群号 == "兜底规则":
            yield event.plain_result("❌ 兜底规则不可删除，如需关闭请使用 /设置群规 开关 off 兜底规则")
            return

        规则列表 = self.config.get('自定义规则', [])
        if not 规则列表:
            yield event.plain_result("⚠️ 你还没有配置任何自定义规则")
            return
        目标规则 = None
        目标索引 = -1
        for idx, 规则 in enumerate(规则列表):
            if 群号 in 规则['群号']:
                目标规则 = 规则
                目标索引 = idx
                break

        if 目标规则 is None:
            yield event.plain_result(f"❌ 未找到群 {群号} 的自定义规则")
            return

        当前群号列表 = 目标规则['群号']

        if 群号 == "无":
            被删规则 = 规则列表.pop(目标索引)
            self.config['自定义规则'] = 规则列表
            self.config.save_config()
            for gid in 被删规则['群号']:
                if gid != "无":
                    self.群组活跃时间.pop(gid, None)
                    self.群组上次唤醒时间.pop(gid, None)
                    self.群组活跃间隔.pop(gid, None)
                    self.用户活跃时间.pop(gid, None)
                    self.用户上次唤醒时间.pop(gid, None)
            self._重建规则索引()
            yield event.plain_result(f"✅ 已永久删除空规则（原备注：{被删规则['备注']}）")
            return

        新群号列表 = [g for g in 当前群号列表 if g != 群号]

        # 清理该群的运行时状态（群聊级和用户级都清理一下）
        self.群组活跃时间.pop(群号, None)
        self.群组上次唤醒时间.pop(群号, None)
        self.群组活跃间隔.pop(群号, None)
        # 用户级状态是按用户ID的，这里不清理全局用户状态，因为其他群可能用到

        if 新群号列表:
            目标规则['群号'] = 新群号列表
            self.config.save_config()
            self._重建规则索引()
            剩余群 = '、'.join(新群号列表)
            yield event.plain_result(
                f"✅ 已将群 {群号} 从规则中移除。\n"
                f"规则仍绑定群号：{剩余群}\n"
                f"备注：{目标规则['备注']}"
            )
        else:
            目标规则['群号'] = ["无"]
            self.config.save_config()
            self._重建规则索引()
            yield event.plain_result(
                f"⚠️ 已从规则「{目标规则['备注']}」移除群{群号}，该规则已无绑定群号。\n\n"
                f"若确认不再需要，请执行 `/删除群规 无`，删除整条规则\n\n"
                f"可使用`/设置群规 群号 {群号} 无 rep`以恢复"
            )

    @filter.command("群规帮助")
    @filter.permission_type(filter.PermissionType.ADMIN)
    async def 群规帮助(self, event: AstrMessageEvent):
        """显示群自定义规则插件的帮助文档"""
        yield event.plain_result(帮助文本)

    @filter.command("设置活跃")
    async def 设置活跃(self, event: AstrMessageEvent, 秒数: str):
        """为自己设置活跃时间（最大600秒），群组内隔离"""
        群号 = event.get_group_id()
        if not 群号:
            yield event.plain_result("❌ 请在群聊中使用")
            return
        try:
            秒数 = int(秒数)
        except (ValueError, TypeError):
            yield event.plain_result("❌ 请输入有效的秒数数字")
            return
        if 秒数 <= 0:
            yield event.plain_result("❌ 秒数必须大于0")
            return
        if 秒数 > 600:
            yield event.plain_result("❌ 最大支持600秒（10分钟）")
            return
        发送者 = event.get_sender_id()
        self.用户自定义活跃时长[(群号, 发送者)] = 秒数
        yield event.plain_result(f"✅ 已设置活跃时长{秒数}秒，唤醒后将保持活跃并自动续期")

    @filter.command("停止活跃")
    async def 停止活跃(self, event: AstrMessageEvent, 清除: str = None):
        """停止活跃状态，传任意参数则同时清除自定义时长设置"""
        群号 = event.get_group_id()
        if not 群号:
            yield event.plain_result("❌ 请在群聊中使用")
            return
        发送者 = event.get_sender_id()
        键 = (群号, 发送者)
        清理了时间 = self.用户自定义活跃时间.pop(键, None)
        if 清除 is not None:
            清理了时长 = self.用户自定义活跃时长.pop(键, None)
            if 清理了时间 or 清理了时长:
                yield event.plain_result("✅ 已停止活跃状态并清除自定义时长")
            else:
                yield event.plain_result("⚠️ 你当前没有自定义活跃状态")
        else:
            if 清理了时间:
                yield event.plain_result("✅ 已停止活跃状态（自定义时长保留）")
            else:
                yield event.plain_result("⚠️ 你当前没有活跃状态")
